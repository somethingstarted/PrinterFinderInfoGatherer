...
# Prepare the row for hostnames
hostnames_row=",Hostnames:"
for ip in "${printer_ips[@]}"; do
  # Fetch the hostname
  echo "Querying hostname for printer: $ip"
  hostname=$(snmpget -v2c -c $community $ip $oid_hostname | awk -F ': ' '{print $2}')
  
  # Output IP and hostname to console
  echo "Hostname: $hostname"
  
  hostnames_row="$hostnames_row,$hostname"
done
