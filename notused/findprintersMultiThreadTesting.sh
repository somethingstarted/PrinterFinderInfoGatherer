...
multithreading=$(yq eval '.multithreading' "$config_file")
max_threads=$(yq eval '.max_threads' "$config_file")
...
scan_ips_in_parallel() {
    ...
    for ip in "${ips[@]}"; do
        echo "$ip"
    done | parallel -j "$max_threads" scan_ip
}

if [[ "$debug_mode" == "true" ]]; then
    # Scan known printers
    if [[ "$multithreading" == "true" && "$max_threads" -gt 1 ]]; then
        scan_ips_in_parallel "${known_printers[@]}"
    else
    ...
    for subnet in "${subnets[@]}"; do
        ...
        if [[ "$multithreading" == "true" && "$max_threads" -gt 1 ]]; then
            scan_ips_in_parallel "${ips_in_subnet[@]}"
        else
